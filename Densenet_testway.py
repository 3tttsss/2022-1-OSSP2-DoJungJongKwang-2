...
valid_datasets=['/content/drive/MyDrive/offline-test.pkl', '/content/drive/MyDrive/2022-1-OSSP2-DoJungJongKwang-2-kdy/test_caption.txt']
    ...
    for i in range(batch_size_t):
        for j in range(maxlen):
            if int(prediction[i][j]) ==0:
                ...
                ####################################################수정부분 2/3
                if int(prediction[i][j]) == 1:
                  prediction_symp.append('>=')
                  time_on = 0;
                elif int(prediction[i][j]) == 2:
                  if time_on == 1:
                    prediction_symp.append('*sqrt')
                  else:
                    prediction_symp.append('sqrt')
                  time_on = 0;
                elif int(prediction[i][j]) == 3:
                  prediction_symp.append('<=')
                  time_on = 0;
                elif int(prediction[i][j]) == 4: #/
                  prediction_symp.append(worddicts_r[int(prediction[i][j])])
                  time_on = 0;
                elif int(prediction[i][j]) == 5:
                  prediction_symp.append('infty')
                  time_on = 0;
                elif int(prediction[i][j]) == 6: #(
                  prediction_symp.append(worddicts_r[int(prediction[i][j])])
                  time_on = 0;
                elif int(prediction[i][j]) == 7: #,
                  prediction_symp.append(worddicts_r[int(prediction[i][j])])
                  time_on = 0;
                elif int(prediction[i][j]) == 9:
                  prediction_symp.append('...')
                  time_on = 0;
                elif int(prediction[i][j]) == 11:
                  prediction_symp.append('(sigma)')
                  time_on = 0;
                elif int(prediction[i][j]) == 12: #<
                  prediction_symp.append(worddicts_r[int(prediction[i][j])])
                  time_on = 0;
                elif int(prediction[i][j]) == 13:
                  prediction_symp.append('+-')
                  time_on = 0;
                elif int(prediction[i][j]) == 14:
                  if time_on == 1:
                    prediction_symp.append('*log')
                  else:
                    prediction_symp.append('log')
                  time_on = 0;
                elif int(prediction[i][j]) == 15:
                  prediction_symp.append('(pi)')
                  time_on = 0;
                elif int(prediction[i][j]) == 19:
                  prediction_symp.append('limits')
                  time_on = 0;
                elif int(prediction[i][j]) == 24: #}
                  prediction_symp.append(worddicts_r[int(prediction[i][j])])
                  time_on = 0;
                elif int(prediction[i][j]) == 28:
                  if time_on == 1:
                    prediction_symp.append('*tan')
                  else:
                    prediction_symp.append('tan')
                  time_on = 0;
                elif int(prediction[i][j]) == 31:
                  prediction_symp.append('(gamma)')
                  time_on = 0;
                elif int(prediction[i][j]) == 32: #{
                  prediction_symp.append(worddicts_r[int(prediction[i][j])])
                  time_on = 0;
                elif int(prediction[i][j]) == 33: #'
                  prediction_symp.append(worddicts_r[int(prediction[i][j])])
                  time_on = 0;
                elif int(prediction[i][j]) == 34: #+
                  prediction_symp.append(worddicts_r[int(prediction[i][j])])
                  time_on = 0;
                elif int(prediction[i][j]) == 35:
                  prediction_symp.append('(theta)')
                  time_on = 0;
                elif int(prediction[i][j]) == 36:
                  prediction_symp.append('(for all)')
                  time_on = 0;
                elif int(prediction[i][j]) == 39:
                  prediction_symp.append('int(')
                  int_on = 1
                  time_on = 0;
                elif int(prediction[i][j]) == 40:
                  if time_on == 1:
                    prediction_symp.append('*sin')
                  else:
                    prediction_symp.append('sin')
                  time_on = 0;
                elif int(prediction[i][j]) == 44:
                  prediction_symp.append('...')
                  time_on = 0;
                elif int(prediction[i][j]) == 46: #[
                  prediction_symp.append(worddicts_r[int(prediction[i][j])])
                  time_on = 0;
                elif int(prediction[i][j]) == 47: #_
                  prediction_symp.append(worddicts_r[int(prediction[i][j])])
                  time_on = 0;
                elif int(prediction[i][j]) == 50:
                  prediction_symp.append('...')
                  time_on = 0;
                elif int(prediction[i][j]) == 56:
                  if time_on == 1:
                    prediction_symp.append('*cos')
                  else:
                    prediction_symp.append('cos')
                  time_on = 0;
                elif int(prediction[i][j]) == 57:
                  if time_on == 1:
                    prediction_symp.append('*(')
                  else:
                    prediction_symp.append('(')
                  time_on = 0;
                elif int(prediction[i][j]) == 58:
                  prediction_symp.append('(delta)')
                  time_on = 0;
                elif int(prediction[i][j]) == 59:
                  prediction_symp.append('!=')
                  time_on = 0;
                elif int(prediction[i][j]) == 60:
                  prediction_symp.append('(in)')
                  time_on = 0;
                elif int(prediction[i][j]) == 61:
                  prediction_symp.append('(alpha)')
                  time_on = 0;
                elif int(prediction[i][j]) == 62:
                  prediction_symp.append('*')
                  time_on = 0;
                elif int(prediction[i][j]) == 63:
                  prediction_symp.append('lim')
                  time_on = 0;
                elif int(prediction[i][j]) == 64: #.
                  prediction_symp.append(worddicts_r[int(prediction[i][j])])
                  time_on = 0;
                elif int(prediction[i][j]) == 67:
                  prediction_symp.append('(lambda)')
                  time_on = 0;
                elif int(prediction[i][j]) == 71:
                  prediction_symp.append('(exists)')
                  time_on = 0;
                elif int(prediction[i][j]) == 75:
                  prediction_symp.append('**')
                  time_on = 0;
                elif int(prediction[i][j]) == 82: #>
                  prediction_symp.append(worddicts_r[int(prediction[i][j])])
                  time_on = 0;
                elif int(prediction[i][j]) == 83:
                  frac_on = 1
                elif int(prediction[i][j]) == 84:
                  prediction_symp.append('->')
                  time_on = 0;
                elif int(prediction[i][j]) == 85:
                  prediction_symp.append('/')
                  time_on = 0;
                elif int(prediction[i][j]) == 86: #!
                  prediction_symp.append(worddicts_r[int(prediction[i][j])])
                  time_on = 0;
                elif int(prediction[i][j]) == 87:
                  prediction_symp.append('(phi)')
                  time_on = 0;
                elif int(prediction[i][j]) == 88: #)
                  prediction_symp.append(worddicts_r[int(prediction[i][j])])
                  time_on = 0;
                elif int(prediction[i][j]) == 89: #-
                  prediction_symp.append(worddicts_r[int(prediction[i][j])])
                  time_on = 0;
                elif int(prediction[i][j]) == 93: #=
                  prediction_symp.append(worddicts_r[int(prediction[i][j])])
                  time_on = 0;
                elif int(prediction[i][j]) == 96:
                  prediction_symp.append('(beta)')
                  time_on = 0;
                elif int(prediction[i][j]) == 100: #]
                  prediction_symp.append(worddicts_r[int(prediction[i][j])])
                  time_on = 0;
                elif int(prediction[i][j]) == 101:
                  prediction_symp.append('(mu)')
                  time_on = 0;
                elif int(prediction[i][j]) == 105:
                  prediction_symp.append('sum')
                  time_on = 0;
                elif int(prediction[i][j]) == 110:
                  prediction_symp.append(')')
                  if frac_on == 1:
                    prediction_symp.append('/')
                    frac_on = 0
                  time_on = 0;
                elif int(prediction[i][j]) == 8 or 90 or 65 or 37 or 68 or 91 or 66 or 38 or 10 or 92:
                  prediction_symp.append(worddicts_r[int(prediction[i][j])])
                  time_on = 1
                else: #문자
                  if int(prediction[i][j]) == 22:
                    if int_on == 1:
